...
class RepositoryVisualizer(param.Parameterized):
    ...
    def visualize(self, event=None):
        ...
        try:
            ...
            self.window_title = old_title  # Update reactive window title
    ...
    def set_window_title(self):
        ...
        # Only update the template title once
        pn.state.template.param.update(title=self.window_title)
...
function_selector = pn.widgets.MultiSelect(
    name="Select Functions to Visualize, empty for all",
...
def reset_camera(event):
    ...
    # vtkpan.object = None  # Force a refresh
...
reset_camera_button = pn.widgets.Button(
    name="Reset Camera",
    button_type="primary",
    width=200,  # Set fixed width for the button
)
...
def update_status(event):
    ...
    # Only update the window title when visualization is complete
    if status.startswith("Visualization saved to"):
...
button_row = pn.Row(
    pn.Column(visualize_button, width=200, sizing_mode="fixed", margin=(0, 10, 0, 0)),
    pn.Column(
        reset_camera_button, width=200, sizing_mode="fixed", margin=(0, 10, 0, 0)
    ),
    pn.Column(status_display, sizing_mode="stretch_width"),
...
# Create main layout
main_layout = pn.Row(visualization_column)
...
# We don't need this binding since we're directly updating the template title in set_window_title
# pn.bind(lambda title: template.param.update(title=title), visualizer.param.window_title)
